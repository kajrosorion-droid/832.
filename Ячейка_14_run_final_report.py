...
try:
    if hasattr(engine, 'core_chorus') and engine.core_chorus is not None:
        if '_save_chorus_state' in globals():
            # ПАТЧ 3d/8e: культура и сложность среды сохраняются вместе с мудростью Хора
            engine.core_chorus.persistent['culture_ratchet'] = engine.culture_ratchet
            ...
            print(f"📜 Состояние Хора сохранено: диалогов={engine.core_chorus.persistent['total_dialogues']}, "
                  ...
                  f"культура={engine.culture_ratchet}, среда={engine.env_complexity:.2f}")
        else:
