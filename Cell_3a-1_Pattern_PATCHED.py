        ...
        if sim > Config.DEEP_EXCHANGE_SIM_THRESHOLD and (
            ...
            for sig, data in top_other:
                if sig not in self.concept_graph.nodes:
                    ...
                    # ДОБАВЛЕНО (найдено по жалобе на монокультуру 'reciprocity',
                    # 88% популяции носители): перенос был ПРОПОРЦИОНАЛЕН текущему
                    # размеру концепта у донора -- классический rich-get-richer.
                    # Тот же паттерн демпфирования, что уже работает в TrustLedger
                    # (headroom = 1.0 - current, delta *= headroom), применённый
                    # здесь к потолку 1000.0 (уже существующему ниже, в decay-цикле).
                    _cur = self.concept_graph.nodes[sig]['count']
                    _headroom = max(0.0, 1.0 - _cur / 1000.0)
                    ...
                    if self.concept_graph.nodes:
                        ...
                        if neighbor_id:
                            ...
                            if sig not in neighbor.concept_graph.nodes:
                                ...
                                # ДОБАВЛЕНО: тот же демпфер, что и в deep_exchange --
                                # эта ветка (наследие при смерти-пророчестве) реже,
                                # но множитель ещё больше (0.5), так что усиливает
                                # ту же монокультуру, если её тоже не гасить.
