...
class Pattern:
    ...
    def update_model_part1(self, field, t, witness):
        ...
        if neighbor_count > 0:
            ...
            complex_neighbors = 0
            # БАГФИКС (найдено по KeyboardInterrupt-трейсбеку): n_gap зависит
            # только от neighbor_p.prediction/belief, не от того, какая клетка
            # self на него смотрит. У гигантских паттернов (до GIANT_CELL_LIMIT
            # клеток) один и тот же сосед мог пересчитываться десятки раз за
            # вызов -- каждый раз новый np.mean(np.abs(...)). Кэшируем по id
            # соседа: значение и порядок подсчёта complex_neighbors (по числу
            # клеток-соседств, как и раньше) не меняются, меняется только
            # число реальных numpy-вызовов.
            _neighbor_gap_cache = {}
            for (x,y) in self.cells:
                for dx, dy in [(-1,0),(1,0),(0,-1),(0,1)]:
                    nx, ny = (x+dx)%Config.WORLD_SIZE, (y+dy)%Config.WORLD_SIZE
                    ...
                    if owner != 0 and owner != self.id and owner in self.world.pattern_dict:
                        ...
                        if neighbor_p.alive:
                            if owner in _neighbor_gap_cache:
                                n_gap = _neighbor_gap_cache[owner]
                            else:
                                n_gap = float(np.mean(np.abs(neighbor_p.prediction - neighbor_p.belief)))
                                _neighbor_gap_cache[owner] = n_gap
        ...
        if sim > Config.DEEP_EXCHANGE_SIM_THRESHOLD and (
            ...
            for sig, data in top_other:
                if sig not in self.concept_graph.nodes:
                    ...
                    # ДОБАВЛЕНО: тот же приём diminishing-returns, что уже
                    # работает в TrustLedger (headroom = 1 - current). Перенос
                    # раньше был data['count']*0.2 БЕЗ разбора, насколько
                    # концепт уже доминирует у получателя -- это и есть
                    # реальный движок monокультуры (см. чат): чем больше
                    # count, тем больше абсолютная прибавка на каждом обмене.
                    # Выключено по умолчанию (ENABLE_CONCEPT_TRANSFER_DAMPING).
                    if getattr(Config, 'ENABLE_CONCEPT_TRANSFER_DAMPING', False):
                        _ceiling = getattr(Config, 'CONCEPT_TRANSFER_CEILING', 300.0)
                        _headroom = max(0.05, 1.0 - min(1.0, self.concept_graph.nodes[sig]['count'] / _ceiling))
            ...
            for sig, data in top_self:
                if sig not in other.concept_graph.nodes:
                    ...
                    if getattr(Config, 'ENABLE_CONCEPT_TRANSFER_DAMPING', False):
                        _ceiling = getattr(Config, 'CONCEPT_TRANSFER_CEILING', 300.0)
                        _headroom = max(0.05, 1.0 - min(1.0, other.concept_graph.nodes[sig]['count'] / _ceiling))
    ...
    def form_concepts(self):
        ...
        # ДОБАВЛЕНО: "налог на истину" против монокультуры одного концепта
        # (см. чат: reciprocity freq=1000.00 во всех прогонах). Отдельно
        # от decay_edges выше -- не завязан на coherence, работает всегда,
        # пока включён. По умолчанию выключен, поведение не меняется.
        if getattr(Config, 'ENABLE_SEMANTIC_FATIGUE', False) and self.age % 10 == 0:
            self.concept_graph.apply_semantic_fatigue(
                dominance_share=getattr(Config, 'SEMANTIC_FATIGUE_SHARE', 0.35),
                decay_rate=getattr(Config, 'SEMANTIC_FATIGUE_DECAY', 0.9))
