...
class EvolutionEngine:
    ...
    def create_pattern(self, cells, parent=None):
        ...
        # ПАТЧ 2e: рождение в обжитой нише даёт бонус к soul_weight
        n0 = safe_mean([self.niche[c[0], c[1]] for c in cells], 0) if hasattr(self, 'niche') else 0
        if n0 > 0.3:
            p.soul_weight = min(1.0, p.soul_weight + 0.05)
            p._log_event("born_in_niche", niche=round(n0, 2))
    ...
    def field_dynamics(self, t):
        ...
        if hasattr(self, 'env_complexity'):
    ...
    def spawn_unknown_patterns(self, t, max_new=None):
        ...
        prob *= getattr(self, 'env_complexity', 1.0)  # ПАТЧ 8d: Красная Королева
        new_count = 0
        for x in range(Config.WORLD_SIZE):
            for y in range(Config.WORLD_SIZE):
    ...
    def step(self, t):
        ...
        for p in self.patterns:
            ...
            regen *= (1 + 0.2 * getattr(p, '_phi_proxy', 0))  # ПАТЧ 5b
    ...
    def run(self, steps=None, save_ark=True, force_fresh_seed=False):
        ...
        self.niche = np.zeros((Config.WORLD_SIZE, Config.WORLD_SIZE))  # ПАТЧ 2a
        # ПАТЧ 3d/8e: персистентность между прогонами через тот же механизм,
        # что и мудрость Хора (chorus_state.json) — культура и сложность
        # среды переживают отдельный прогон, а не обнуляются каждый раз.
        if hasattr(self, 'core_chorus') and self.core_chorus is not None and hasattr(self.core_chorus, 'persistent'):
            _cs = self.core_chorus.persistent
            self.culture_ratchet = _cs.get('culture_ratchet', 0)
            self.env_complexity = _cs.get('env_complexity', 1.0)
            self._teach_prev = 0
        self._total_energy_last = float(np.sum(self.field[:,:,CH['energy']]))
        self.patterns = []
        self.pattern_dict = {}
