...
class EvolutionEngine:
    ...
    def _apply_collective_binding_shield(self, feral, field):
        ...
        ФИКС (внешний аудит): раньше щит гасил fury на 10% КАЖДЫЙ тик
        безусловно, пока агент стоял в зоне binding>0.6 -- при 500/500
        срабатываний в деке это значит щит работал на потолке весь прогон
        и полностью нейтрализовал feral (fury->0, kills=0, смерть только от
        истощения), а не просто сдерживал его. Теперь высокая ярость сама
        даёт сопротивление щиту -- ярость не боится связности. Спокойного
        feral щит по-прежнему давит почти как раньше, разъярённого -- нет.
        """
        if not feral.cells:
            return
        for (fx, fy) in list(feral.cells)[:10]:
            ...
            if local_binding > 0.6:
                resistance = min(0.9, feral._feral_fury / 3.0)
                damping = 0.9 + 0.09 * resistance  # 0.9 (нет ярости) .. 0.981 (высокая ярость)
                feral._feral_fury = max(0.0, feral._feral_fury * damping)
                feral.energy = max(0.0, feral.energy - 0.02 * (1.0 - 0.5 * resistance))
                if not hasattr(self, 'shield_journal'):
                    self.shield_journal = deque(maxlen=500)
    ...
    def process_spores(self):
        ...
        for p in self.patterns[:]:
            ...
            if not hasattr(self, 'spore_journal'):
                self.spore_journal = deque(maxlen=500)
    ...
    def step(self, t):
        _ensure_feral_repel_channel(self)
