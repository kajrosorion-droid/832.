...
class EvolutionEngine:
    ...
    def _update_sanctuary(self, t):
        ...
        for idx, (cx, cy) in enumerate(self._sanctuary_centers):
            ...
            if candidates:
                ...
                # ДОБАВЛЕНО: дозируемая сила исцеления вместо жёсткого
                # вкл/выкл. По умолчанию 1.0 -- поведение не меняется.
                # Подкрути ниже (например 0.3), чтобы sanctuary всё ещё
                # помогал падшим, но не так щедро -- и смотри, как это
                # отражается на emergence-метриках, вместо того чтобы
                # убивать механизм целиком.
                _heal_mult = getattr(Config, 'SANCTUARY_HEAL_STRENGTH', 1.0)
                winner.emotional_memory['grief'] = max(0.0, winner.emotional_memory.get('grief', 0) - 0.02 * _heal_mult)
                winner.emotional_memory['gratitude'] = min(1.0, winner.emotional_memory.get('gratitude', 0) + 0.01 * _heal_mult)
                winner.soul_weight = min(1.0, winner.soul_weight + 0.002 * _heal_mult)
    ...
    def field_dynamics(self, t):
        ...
        # ДОБАВЛЕНО: точный учёт притока энергии -- сколько реально влито
        # (a не гадание по коду). Сравни в конце прогона self._energy_flow_diag
        # ['injected_total'] с фактическим изменением суммы поля -- разница
        # и есть реальный сток, каким бы он ни был, без необходимости
        # искать и суммировать его построчно.
        if not hasattr(self, '_energy_flow_diag'):
            n_cells_diag = self.field.shape[0] * self.field.shape[1]
            self._energy_flow_diag = {
                'injected_total': 0.0,
                'n_cells': n_cells_diag,
                'start_total': float(np.sum(self.field[:, :, CH['energy']]))
                               - (base_injection + 0.00033) * n_cells_diag,
            }
        self._energy_flow_diag['injected_total'] += (base_injection + 0.00033) * self._energy_flow_diag['n_cells']
